...
(function ($, sp) {
	...
	var SPLanguagePicker = function SPLanguagePicker(element, options) {
		this._defaults = {
	...
	SPLanguagePicker.prototype.getUserLanguages = function () {
		...
		return self._dao.profiles.current().then(function (profile) {
			...
			console.log(profile);
			self.rawUserLanguages = langStr.split(",").map(function (lang) {
				return lang.trim();
			});
	...
	var getMappedLanguagesByLcid = function getMappedLanguagesByLcid(ids) {
		ids = ids.map(function (lcid) {
			return lcid + "";
		});
	...
	var getMappedLanguagesByCode = function getMappedLanguagesByCode(codes) {
		var matches = [];
	...
	SPLanguagePicker.prototype.getLanguageOptions = function () {
		return this._dao.get("/web/supportedUILanguageIds").then(sp.helpers.validateODataV2).then(function (data) {
	...
	SPLanguagePicker.prototype.init = function () {
		...
		self.getLanguageOptions().then(self.renderOptions.bind(self)).then(self.getUserLanguages.bind(self)).then(self.setToUsersLanguage.bind(self)).then(self.bindEvents.bind(self)).fail(function () {
...
//# sourceMappingURL=splanguagepicker.jquery.js.map